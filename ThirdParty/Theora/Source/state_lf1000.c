...
#define HW_IDCT	1
#define HW_SHIFT 2
extern void InitIDCT(void);
extern void DecodeIDCT(signed short* pDataIn, signed short* pDataOut);
extern void DeInitIDCT(void);
extern void IDct10( ogg_int16_t* InputData, ogg_int16_t *QuantMatrix, ogg_int16_t * OutputData );
extern void IDctSlow( ogg_int16_t* InputData, ogg_int16_t *QuantMatrix, ogg_int16_t * OutputData );
...
void oc_state_frag_recon_c(oc_theora_state *_state,oc_fragment *_frag,
 ...
 ogg_uint16_t _dc_iquant, const ogg_uint16_t _ac_iquant[64]){
  ...
  if(_last_zzi<2){
    ...
    dct_buf[0]=(ogg_int16_t)((ogg_int32_t)_frag->dc*_dc_iquant >> HW_SHIFT);
    for(zzi=1;zzi<_ncoefs;zzi++){
      int ci;
      ci=OC_FZIG_ZAG[zzi];
      dct_buf[ci]=(ogg_int16_t)((ogg_int32_t)_dct_coeffs[zzi]*_ac_iquant[ci] >> HW_SHIFT);
    }
...
#else
    if (_last_zzi < 10)
        IDct10(_ac_iquant, _dct_coeffs, res_buf);
    else
    	IDctSlow(_ac_iquant, _dct_coeffs, res_buf);
#endif
  }
