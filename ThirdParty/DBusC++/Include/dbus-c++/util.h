...
// Could not find any other macro to indicate
// the availability  of __sync_sub_and_fetch.
// assuming that if __GCC_HAVE_SYNC_COMPARE_AND_SWAP_4
// then the __sync_sub_and_fetch is available as well.
...
private:
  ...
  {
    #if USE_GCC_ATOMICS
    (void)__sync_add_and_fetch(__ref,1);
    #else
    ++ (*__ref);
    #endif
  }
  DXXAPILOCAL void unref() const
  {
    ...
    #if USE_GCC_ATOMICS
    int refcnt = __sync_sub_and_fetch(__ref, 1);
