 ...
 */
static void SS_UpdateInfo( HybridSynth_t *hybridSynth, const HybridVoice_Preset_t *preset, HybridVoice_Info_t *info )
{
	info->preset = preset;
...
static HybridVoice_Info_t *SS_FindOrAllocateInfo( HybridSynth_t *hybridSynth, const HybridVoice_Preset_t *preset )
{
	...
	if( match == NULL )
	{
		if( empty != NULL )
		{
			match = empty;
			empty->numUsers = 0;
			SS_UpdateInfo( hybridSynth, preset, empty );
		}
	}
...
/********************************************************************/
static void SS_StartVoice( HybridSynth_t *hybridSynth, HybridVoice_t *voice, const HybridVoice_Preset_t *preset,
                       ...
                       int pitch, int velocity, int pan )
{
	...
	velocity = SS_ApplyVelocityEQ( hybridSynth, synthPitch, velocity );
	...
	SS_StartVoice( hybridSynth, voice, preset, channelIndex, synthPitch, velocity14, pan );
	SS_SetBasePitch( hybridSynth, voice, synthPitch, velocity );
}
...
*/
	...
	SS_StartVoice( hybridSynth, voice, preset, channelIndex, pitch, velocity14, pan );
...
static int SS_GetParameter( SoftSynth *synth, SPMIDI_Parameter parameterIndex, int *valuePtr )
{
	...
	case SPMIDI_PARAM_RHYTHM_VOLUME:
		result = hybridSynth->softSynth.rhythmVolume;
		break;
...
{
	(void) spmidiContext; /* TODO handle per context instruments, move ins manager to spmidiContext? */
	return InsManager_Add( &sHybridSynthShared.insManager, instrument,
	                       bankIndex, programIndex );
}
