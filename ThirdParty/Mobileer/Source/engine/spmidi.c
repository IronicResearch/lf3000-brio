...
static void SPMIDI_VoiceOn( SPMIDIContext_t *spmc, SPMIDIVoiceTracker_t *voiceTracker,
                            int channelIndex, int pitch, int velocity )
{
	SPMIDIChannel_t *channel = &spmc->channels[ channelIndex ];
	...
	if( channel->isDrum )
	{
		...
		/* Clip to legal range. */
		if( pitch < GMIDI_FIRST_DRUM )
			pitch = GMIDI_FIRST_DRUM;
		else if( pitch > GMIDI_LAST_DRUM )
			pitch = GMIDI_LAST_DRUM;

		/* Each drum has a specific pan setting. */
		drumIndex = pitch - GMIDI_FIRST_DRUM;
		pan = sDrumPan[ drumIndex ];
		voiceTracker->exclusionGroup = sDrumExclusionGroups[ drumIndex ];

		spmc->synth->TriggerDrum( spmc->synth, voiceTracker->id, channelIndex, pitch, velocity, pan );
	}
	else
	{
		spmc->synth->NoteOn( spmc->synth, voiceTracker->id, channelIndex,
		                     pitch, velocity, channel->pan );
	}
 ...
 */
static SPMIDIVoiceTracker_t *SPMIDI_AllocateVoice( SPMIDIContext_t *spmc, int channelIndex, int pitch )
{
	...
	DLL_FOR_ALL( SPMIDIVoiceTracker_t, next, &channel->voiceList )
	{
		 ...
		 */
		if( (next->pitch == pitch) && (next->isStifled == FALSE) )
		{
			voiceToStifle = next;
			break;
		}
 ...
 */
static void SPMIDI_CallVibrator( SPMIDIContext_t *spmc, int pitch, int velocity )
{
	if( spmc->vibratorCallback != NULL )
	{
		(*(spmc->vibratorCallback))( spmc->vibratorUserData, pitch, velocity );
	}
}
...
static void SPMIDI_HandleNoteOn( SPMIDIContext_t *spmc, int channelIndex, int pitch, int velocity )
{
	if( velocity == 0 )
	{
		SPMIDI_HandleNoteOff( spmc, channelIndex, pitch, velocity );
	}
	else
	{
		SPMIDIChannel_t *channel = &spmc->channels[channelIndex];
		...
		{
			SPMIDI_CallVibrator( spmc, pitch, velocity );
		}
		else
		{
			if( !channel->isMuted && !channel->isDisabled )
			{
				SPMIDIVoiceTracker_t *voiceTracker = SPMIDI_AllocateVoice( spmc, channelIndex, pitch );
				/* Did we get a voice? If not then it must not have been an important channel. */
				if( voiceTracker != NULL )
				{
					SPMIDI_VoiceOn( spmc, voiceTracker, channelIndex, pitch, velocity );
				}
			}
		}
	}
...
static void SPMIDI_HandleNoteOff( SPMIDIContext_t *spmc, int channelIndex, int pitch, int velocity )
{
	...
	if( channel->isVibrating )
	{
		SPMIDI_CallVibrator( spmc, pitch, 0 );
		return;
	}
	...
	{
		if( (voice->pitch == pitch) && voice->isOn && !voice->isSustaining)
		{
...
static void SPMIDI_AllNotesOff( SPMIDIContext_t *spmc, int channelIndex )
{
	...
	DLL_FOR_ALL( SPMIDIVoiceTracker_t, voice, &channel->voiceList )
	{
		SPMIDI_HandleNoteOff( spmc, channelIndex, voice->pitch, 0 );
	}
}
