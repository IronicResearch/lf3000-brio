...
static Boolean InitCameraBufferInt(tCameraContext *pCamCtx)
{
	...
	for(i = 0; i < pCamCtx->numBufs; i++)
	{
		...
		pCamCtx->buf.length	   = (pCamCtx->mode.pixelformat == kCaptureFormatRAWYUYV) ? 2 * pCamCtx->mode.width * pCamCtx->mode.height : 4096 * ((pCamCtx->mode.height + 0xFF) & ~0xFF);
...
static Boolean ReturnFrameInt(tCameraContext *pCtx, const U32 index)
{
	...
	buf.length    = (pCtx->mode.pixelformat == kCaptureFormatRAWYUYV) ? 2 * pCtx->mode.width * pCtx->mode.height : 4096 * ((pCtx->mode.height + 0xFF) & ~0xFF);
#endif
