...
avi_t* AVI_open_output_file(char * filename, bool audio)
{
	...
	AVOutputFormat *fmt = av_guess_format("avi", filename, NULL);
	if (fmt == NULL)
		return NULL;
    ...
    if (avio_open(&avi->pFormatCtx->pb, avi->pFormatCtx->filename, URL_WRONLY) < 0)
    	return NULL;

#if 0 // FIXME: non-blocking mode
...
{
	int64_t	filepos = avio_tell(avi->pFormatCtx->pb);
	
	// Rewind file if rewriting AVI header
	if (rewrite)
		avio_seek(avi->pFormatCtx->pb, 0, SEEK_SET);

	// Write AVI header
	avformat_write_header(avi->pFormatCtx, NULL);
	
	// Reposition file after rewriting AVI header
	if (rewrite)
		avio_seek(avi->pFormatCtx->pb, filepos, SEEK_SET);
	
	av_dump_format(avi->pFormatCtx, 0, avi->pFormatCtx->filename, 1);
}
...
//		write_header(AVI, true);
	...
	int r = avcodec_open2(AVI->pVideoCodecCtx = c, AVI->pVideoCodec, NULL);
	if (r < 0)
		return;
