...
{
{static long c=0; printf("Mix2_Shortsf %ld : start\n", c++);}
...
if	(printFormat == kPrintFormat_Short)
    sprintf(out, "%8gHz %s",    frequency, s);
else
...
printf("FXP31_MULT: afx=$%08lX * bfx=$%08lX  = cfx=$%08lX\n", afx, bfx, cfx);
...
printf("MIX_SCALE_SHIFT_ADD: (%g * %g)>>%ld) =%g\n", af, bf, shiftValue, Q31ToFloat(efx));
printf("MIX_SCALE_SHIFT_ADD: OUT %g + ((%g * %g)>>%ld) = %g \n", df, af, bf, shiftValue, cf);
