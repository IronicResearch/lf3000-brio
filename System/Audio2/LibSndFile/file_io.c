...
{	sf_count_t	new_position ;
	...
	{	case SEEK_SET :
		...
		case SEEK_END :
				...
				{	
//					new_position = _lseek (psf->filedes, offset, whence) ;
...
//#define GK_DONT_HAVE FTRUNCATE
//#ifdef GK_DONT_HAVE FTRUNCATE
	{printf("psf_ftruncate(): don't have this function\n");}
//#else
...
//					new_position = _lseeki64 (psf->filedes, offset, whence) ;
					new_position = lseeki64 (psf->filedes, offset, whence) ;
...
//				offset = _lseeki64 (psf->filedes, 0, SEEK_END) + offset ;
				offset = lseeki64 (psf->filedes, 0, SEEK_END) + offset ;
				break ;
...
//	_lseeki64 (psf->filedes, 0, SEEK_END) ;
	lseeki64 (psf->filedes, 0, SEEK_END) ;

//	if ((filelen = _lseeki64 (psf->filedes, 0, SEEK_END)) < 0)
	if ((filelen = lseeki64 (psf->filedes, 0, SEEK_END)) < 0)
	{	psf_log_syserr (psf, errno) ;
		return (sf_count_t) -1 ;
		} ;
...
//		_lseeki64 (psf->filedes, current, SEEK_SET) ;
	if (filelen > current)
		lseeki64 (psf->filedes, current, SEEK_SET) ;
